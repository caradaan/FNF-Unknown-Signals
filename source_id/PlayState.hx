...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
				...
				case 'ikachanStage':
					{
						defaultCamZoom = 0.65;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-600, -250).loadGraphic(Paths.image('Ikachanback'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.3, 0.3);
						bg.active = false;
						add(bg);
	
						var ikachanfront:FlxSprite = new FlxSprite(-740, -450).loadGraphic(Paths.image('Ikachanfront'));
						ikachanfront.setGraphicSize(Std.int(ikachanfront.width * 1.2));
						ikachanfront.updateHitbox();
						ikachanfront.antialiasing = true;
						ikachanfront.scrollFactor.set(1.0, 1.0);
						ikachanfront.active = false;
						add(ikachanfront);
					}
				...
				case 'ikachanStage2':
					{
						defaultCamZoom = 0.65;
						curStage = 'stage';
						var bg:FlxSprite = new FlxSprite(-600, -250).loadGraphic(Paths.image('ottoback2'));
						bg.antialiasing = true;
						bg.scrollFactor.set(0.3, 0.3);
						bg.active = false;
						add(bg);
		
						var ottofront2:FlxSprite = new FlxSprite(-740, -450).loadGraphic(Paths.image('ottofront2'));
						ottofront2.setGraphicSize(Std.int(ottofront2.width * 1.2));
						ottofront2.updateHitbox();
						ottofront2.antialiasing = true;
						ottofront2.scrollFactor.set(1.0, 1.0);
						ottofront2.active = false;
						add(ottofront2);
					}
				...
				case 'takochanStage':
					{
							defaultCamZoom = 0.65;
							curStage = 'takochan';
							var bg:FlxSprite = new FlxSprite(-600, -250).loadGraphic(Paths.image('ottoback'));
							bg.antialiasing = true;
							bg.scrollFactor.set(0.3, 0.3);
							bg.active = false;
							add(bg);
		
							var ottofront:FlxSprite = new FlxSprite(-740, -450).loadGraphic(Paths.image('ottofront'));
							ottofront.setGraphicSize(Std.int(ottofront.width * 1.2));
							ottofront.updateHitbox();
							ottofront.antialiasing = true;
							ottofront.scrollFactor.set(1.0, 1.0);
							ottofront.active = false;
							add(ottofront);

							//var idFog:FlxSprite = new FlxSprite(-260, 145);//-260, -145
							//idFog.frames = Paths.getSparrowAtlas('idfog');
							//idFog.setGraphicSize(Std.int(idFog.width * 1.2));
							//idFog.alpha = 0.5;
							//idFog.animation.addByPrefix('idfog', "idfog", 13);
							//idFog.animation.play('idfog');
							//idFog.scrollFactor.set(0.9, 0.9);
							//add(idFog);
					}
				case 'philly':
					{
		...
		if (curStage == 'takochan')
			{
				var idFog:FlxSprite = new FlxSprite(-312, 600);//-260, -145
				idFog.frames = Paths.getSparrowAtlas('idfog');
