...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'ikachanStage': //Week id
			defaultCamZoom = 0.65;
			var idbg:BGSprite = new BGSprite('idback1', -600, -250, 0.3, 0.3);
			add(idbg);

			var idfront:BGSprite = new BGSprite('idfront1', -740, -450, 1.0, 1.0);
			idfront.setGraphicSize(Std.int(idfront.width * 1.2));
			idfront.updateHitbox();
			add(idfront);

			case 'idFogStage': //Week id2
			defaultCamZoom = 0.65;
			var idbg2:BGSprite = new BGSprite('idback2', -600, -250, 0.3, 0.3);
			add(idbg2);

			var idfront2:BGSprite = new BGSprite('idfront2', -740, -450, 1.0, 1.0);
			idfront2.setGraphicSize(Std.int(idfront2.width * 1.2));
			idfront2.updateHitbox();
			add(idfront2);

			
			case 'takochanStage': //Week otto
			defaultCamZoom = 0.65;
			var ottobg:BGSprite = new BGSprite('ottoback', -600, -250, 0.3, 0.3);
			add(ottobg);

			var ottofront:BGSprite = new BGSprite('ottofront', -740, -450, 1.0, 1.0);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'uni':
					startDialogue(dialogueJson, 'GreenAbyss');
		...
		if(isDad) {
			...
			switch (curStage)
			{
				...
				case 'school' | 'schoolEvil':
					...
					camFollow.y = boyfriend.getMidpoint().y - 200;
				case 'ikachanStage' | 'idFogStage' | 'takochanStage'://id otto stage
					camFollow.x = boyfriend.getMidpoint().x - 200;
					camFollow.y = boyfriend.getMidpoint().y - 200;
			}
